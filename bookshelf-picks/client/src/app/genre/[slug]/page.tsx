...
import { GENRES, type GenreKey } from "../../../data/genres";
import { motion } from "framer-motion";
import BookService from "@/services/Book.service";
import { Book } from "@/types/book.type";
import parse from "html-react-parser";
import decodeHTMLEntities from "@/utils/decodeHtml";
import getRandomQuoteIndex from "@/utils/getRandomNumber";
import quotes from "@/data/quotes";

const statusTag = (s: Book["status"]) =>
  s === "read" ? (
...
export default function GenrePage() {
  ...
  const [filterStatus, setFilterStatus] = useState<Book["status"] | "all">(
    "all",
  );
  ...
  const cardMenu = (book: Book): MenuProps["items"] => [
    {
      key: "read",
      label: "Mark as Read",
      onClick: () => changeStatus(book._id, "read"),
    },
    {
      key: "want",
      label: "Mark as Want to Read",
      onClick: () => changeStatus(book._id, "want"),
    },
    { type: "divider" },
    {
      key: "details",
      label: <Link href={`/books/${book._id}`}>View details</Link>,
    },
  ];
        ...
        <div className="genreMain">
          ...
          {!isLoading && (
            <List
              ...
              locale={{
                emptyText: (
                  <Empty description={`No ${meta.title} books found`} />
                ),
              }}
              renderItem={(b, i) => (
                <List.Item>
                  ...
                  >
                    <Card
                      ...
                      actions={[
                        ...
                        >
                          {b.favorite ? <HeartFilled /> : <HeartOutlined />}{" "}
